...
//		System.out.println("邻接矩阵构建完毕！");
		//除以每行的总和
//		int[][] mMatrix = gm.getmMatrix();
//		double[][] M = Util.calcDiv(mMatrix);
//		Util.writerTo("C:\\Users\\Administrator\\Desktop\\PR\\gm1.txt",M);
		//得到转置
//		double[][] MT = Util.readTransposeFrom("C:\\Users\\Administrator\\Desktop\\PR\\gm1.txt",nodes.length);
//		Util.writerTo("C:\\Users\\Administrator\\Desktop\\PR\\gmT.txt",MT);
		...
		//AX是下一次的PR值
//		double[] AX = Util.calcAX("C:\\Users\\Administrator\\Desktop\\PR\\gmT.txt",nodes.length,X);
		...
		while(true){
			...
			if(condition(minus(X,AX),0.001)){
				...
				X = AX ;
//				AX = Util.calcAX("C:\\Users\\Administrator\\Desktop\\PR\\gmT.txt",nodes.length,X);
				AX = Util.calcAX(A,X);
				System.out.println("==============");
				a++;
			}
